...
namespace engine::windows
{
	...
	{
		D3D11_QUERY_DESC timestampDisjointDesc = {};
		timestampDisjointDesc.Query = D3D11_QUERY_TIMESTAMP_DISJOINT;

		ID3D11Query* timestampDisjointQuery;
		HRESULT hr = s_device->CreateQuery(&timestampDisjointDesc, &timestampDisjointQuery);
		if (FAILED(hr)) {
			OutputDebugString(TEXT("Failed to create disjoint query."));
			return;
		}
		...
		ID3D11Query* startTimestampQuery;
		hr = s_device->CreateQuery(&timestampDesc, &startTimestampQuery);
		if (FAILED(hr)) {
			OutputDebugString(TEXT("Failed to create start timestamp query."));
			return;
		}

		ID3D11Query* endTimestampQuery;
		hr = s_device->CreateQuery(&timestampDesc, &endTimestampQuery);
		if (FAILED(hr)) {
			OutputDebugString(TEXT("Failed to create end timestamp query."));
			return;
		}

		// Початок вимірювання часу
		s_deviceContext->Begin(timestampDisjointQuery);
		s_deviceContext->End(startTimestampQuery);
		...
		s_deviceContext->End(endTimestampQuery);
		s_deviceContext->End(timestampDisjointQuery);

		// Очікування завершення запитів
		while (s_deviceContext->GetData(timestampDisjointQuery, nullptr, 0, 0) == S_FALSE ||
			s_deviceContext->GetData(startTimestampQuery, nullptr, 0, 0) == S_FALSE ||
			s_deviceContext->GetData(endTimestampQuery, nullptr, 0, 0) == S_FALSE) {
		}

		// Отримання результатів запитів
		D3D11_QUERY_DATA_TIMESTAMP_DISJOINT disjointData;
		hr = s_deviceContext->GetData(timestampDisjointQuery, &disjointData, sizeof(disjointData), 0);
		if (FAILED(hr) || disjointData.Disjoint) {
			OutputDebugString(TEXT("Failed to get disjoint query data or timestamps are disjoint."));
			return;
		}

		UINT64 startTime = 0;
		hr = s_deviceContext->GetData(startTimestampQuery, &startTime, sizeof(startTime), 0);
		if (FAILED(hr)) {
			OutputDebugString(TEXT("Failed to get start timestamp."));
			return;
		}

		UINT64 endTime = 0;
		hr = s_deviceContext->GetData(endTimestampQuery, &endTime, sizeof(endTime), 0);
		if (FAILED(hr)) {
			OutputDebugString(TEXT("Failed to get end timestamp."));
			return;
		}

		// Обчислення часу виконання в мілісекундах
		double time = static_cast<double>(endTime - startTime) / static_cast<double>(disjointData.Frequency) * 1000.0;
		std::string out = "GPU execution time: " + std::to_string(time) + " ms\n";
		OutputDebugStringA(out.c_str());
		

		// Звільнення запитів
		timestampDisjointQuery->Release();
		startTimestampQuery->Release();
		endTimestampQuery->Release();
	}
